...
knitr::opts_chunk$set(echo = TRUE, warnings = FALSE, message = FALSE, eval = FALSE, cache = FALSE)
```
...
# Messgerät ist falsch kalibriert und unterschätzt die Temperatur um 10%
temperaturen_unbiased <- temperaturen * .9
...
```{r}
temperaturen_F <- temperaturen * 5/9 + 32
print(temperaturen_F)
```
...
standardabweichung <- function(x) {
  n <- length(x) 
  mittelwert <- mean(x)
  summe_abweichungsquadrate <- sum((x - mittelwert)^2)
  stabw <- sqrt(summe_abweichungsquadrate / (n - 1))
  stabw
}

temperaturen_sd <- standardabweichung(temperaturen) 
print(temperaturen_sd)
...
- Excel `read::read_xlsx(path = "data/titanic.xlsx")`
...
titanic_xlsx <- readxl::read_xlsx(path = "data/titanic.xlsx")

# Berechnen der Mittelwerte des Alters in der Spalte `age`  
sapply(X = list(titanic_sav$age, titanic_xlsx$age), FUN = mean, na.rm = TRUE)
```
...
Welche 10 Passagiere haben den Höchsten Preis für die Reise bezahlt?

Geben Sie den Namen und den Fahrpreis in der finale Tablle aus.
Wer mag kann überprüfen, wer von diesen Personen überlebt hat.
...
titanic %>% 
  group_by(pclass) %>% 
  summarise(mittelwert = mean(fare, na.rm = TRUE), 
            stabw = sd(fare, na.rm = TRUE),
            n()
  )
```
</div>
